module conv #(
    ...
    rom #(
        ...
        .clk  (clk),
        .ce0  (out_ready),
        .addr0(weight_addr_d0), /* 修改：连接 d0 */
        .q0   (weight_data_d1)  /* 修改：输出连接到 d1 */
    );
    ...
    ram #(
        ...
        .re   (line_buffer_re),
        .raddr(line_buffer_raddr_d1), /* 修改：使用流水线地址 d1 */
        .rdata(line_buffer_rdata_d2)  /* 修改：输出数据 d2 */
    );
            ...
            if (cntr_kk == KK - 1) begin
                ...
                if (cntr_fi == FOLD_I - 1) begin
                    ...
                    if (cntr_fo == FOLD_O - 1) begin
                        cntr_fo <= 0;
                        if (cntr_hw == N_HW - 1) begin /* 修改：修正计数器上限 N_HW - 1 */
                            cntr_hw <= 0;
                        end else begin
                            cntr_hw <= cntr_hw + 1;
                        end
    ...
    always_comb begin
        for (int o = 0; o < P_OCH; o++) begin
            for (int i = 0; i < P_ICH; i++) begin
                w_vec[o][i] = weight_data_d2[(P_ICH*o+i)*W_BIT+:W_BIT]; /* 修改：使用 d2 数据 */
            end
        end
    end
    ...
    generate
        for (genvar o = 0; o < P_OCH; o++) begin : gen_mac_array
            conv_mac_array #(
                ...
                .rst_n  (rst_n),
                .en     (pipe_en_out),
                .dat_vld(mac_array_data_vld_d2), /* 修改：连接流水线后的有效信号 */
                .clr    (is_fst_kk_fi_d2),       /* 修改：连接流水线后的清零信号 */
                .x_vec  (x_vec),
                .w_vec  (w_vec[o]),
                .acc    (acc[o])
            );
