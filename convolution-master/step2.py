...
def MK2K1MK0_DDR2L1(matrix_mk, tensor_m, tensor_k, start_m, start_k, tile_m, tile_k):
    ...
    for k1 in range(K1):
        for m in range(tile_m):
            for k0 in range(K0):
                src_m = start_m + m
                src_k = start_k + k1 * K0 + k0
                if (0 <= src_m < tensor_m) and (0 <= src_k < tensor_k) and (src_k < start_k + tile_k):
                    matrix_k1mk0[k1, m, k0] = matrix_mk[src_m, src_k]
                else:
                    matrix_k1mk0[k1, m, k0] = 0
    return matrix_k1mk0
                ...
                for slice_n_start_in_tile in range(0, n_size_tile, SLICE_N):
                    ...
                    for slice_m_start_in_tile in range(0, m_size_tile, SLICE_M):
                        ...
                        for slice_k_start_in_tile in range(0, k_size_tile_align_k0, SLICE_K):
                            ...
                            psum_en = psum_en = not bias_en
                            deq_en = (tile_k_start_in_tensor + k_size_tile >= TENSOR_K) and \
                                     (slice_k_start_in_tile + k_size_slice >= k_size_tile_align_k0)
