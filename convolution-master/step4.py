...
def BHWC2C1BHWC0_DDR2L1(ifm, batch, tensor_h, tensor_w, tensor_c, start_tensor_h, start_tensor_w, start_tensor_c,
        ...
        for b in range(batch):
            for h in range(tile_h):
                for w in range(tile_w):
                    for c0 in range(C0):
                        if (c1 * C0 + c0) < tile_c:
                            ifm_C1BHWC0[c1][b][h][w][c0] = ifm[b][start_tensor_h + h][start_tensor_w + w][
                                start_tensor_c + c1 * C0 + c0]
...
def matmul_m1k1m0k0_n1k1n0k0(m1n1m0n0, m1k1m0k0, n1k1n0k0, bias_n1n0, deq_n1n0, M1, N1, K1, bias_en, psum_en, deq_en):
    ...
    for m1 in range(M1):
        for n1 in range(N1):
            temp = np.zeros((M0, N0))
            for k1 in range(K1):
                temp += np.matmul(m1k1m0k0[m1][k1], n1k1n0k0[n1][k1].T)
...
def C1BHWC02M1K1M0K0_L12L0(ifm, batch, slice_oh, slice_ow, slice_c1, start_tile_h, start_tile_w, start_tile_c1,
                           kernel_h, kernel_w, stride_h, stride_w, pad_t, pad_b, pad_l, pad_r, dilation_h, dilation_w,
                           padding_value=0):
                ...
                for b in range(batch):
                    for oh in range(slice_oh):
                        for ow in range(slice_ow):
                            ih = start_tile_h - pad_t + oh * stride_h + kh * dilation_h
                            iw = start_tile_w - pad_l + ow * stride_w + kw * dilation_w
                            m_index = b * slice_oh * slice_ow + oh * slice_ow + ow
                            k1_idx = ic1 * kernel_h * kernel_w + kh * kernel_w + kw
                            m1_idx = m_index // M0
                            m0_idx = m_index % M0
                            if (0 <= ih < ifm.shape[2]) and (0 <= iw < ifm.shape[3]):
                                ifm_m1k1m0k0[m1_idx][k1_idx][m0_idx] = ifm[start_tile_c1 + ic1][b][ih][iw]
                            else:
...
def test_convolution():
    ...
    for tile_oc_start_in_tensor in range(0, out_channel, CO_L1):
        for tile_oh_start_in_tensor in range(0, out_height, H_L1):
            for tile_ow_start_in_tensor in range(0, out_width, W_L1):
                ...
                result_m2n2m1n1m0n0_psb = np.zeros(
                    (batch * ceil_div(oh_size_tile, H_L0) * ceil_div(ow_size_tile, W_L0) * ceil_div(oc_size_tile,
                                                                                                    CO_L0), M1, N1, M0,
                     N0))
                for tile_ic_start_in_tensor in range(0, in_channel, CI_L1):
                    ...
                    tile_ih_end_in_tensor = min(tile_oh_end_in_tensor * stride_h - pad_h + dilation_h * (kernel_h - 1),
                                                in_height - 1)
                    tile_iw_end_in_tensor = min(tile_ow_end_in_tensor * stride_w - pad_w + dilation_w * (kernel_w - 1),
                                                in_width - 1)
                    ...
                    ifm_C1BHWC0_tile = BHWC2C1BHWC0_DDR2L1(ifm_BHWC, batch, in_height, in_width, in_channel,
                                                           tile_ih_start_in_tensor, tile_iw_start_in_tensor,
                                                           tile_ic_start_in_tensor, ih_size_tile, iw_size_tile,
                                                           ic_size_tile)
                    weight_k1nk0_tile = OCICKhKw2IC1KhKwOIC0(weight, out_channel, in_channel, kernel_h, kernel_w,
                                                             tile_oc_start_in_tensor, tile_ic_start_in_tensor,
                                                             oc_size_tile, ic_size_tile)
                        ...
                        for slice_oh_start_in_tile in range(0, oh_size_tile, H_L0):
                            for slice_ow_start_in_tile in range(0, ow_size_tile, W_L0):
                                for slice_ci1_start_in_tile in range(0, ci1_size_tile, CI1_L0):
                                    ...
                                    slice_oh_end_in_tensor = min(slice_oh_start_in_tensor + H_L0,
                                                                 tile_oh_start_in_tensor + oh_size_tile) - 1
                                    slice_ow_end_in_tensor = min(slice_ow_start_in_tensor + W_L0,
                                                                 tile_ow_start_in_tensor + ow_size_tile) - 1
                                    ...
                                    ih_end = slice_oh_end_in_tensor * stride_h - pad_h + (kernel_h - 1) * dilation_h
                                    if (ih_end >= in_height):
                                        slice_pad_b = ih_end - in_height + 1
                                    else:
                                        slice_pad_b = 0
                                    ...
                                    iw_end = slice_ow_end_in_tensor * stride_w - pad_w + (kernel_w - 1) * dilation_w
                                    if (iw_end >= in_width):
                                        slice_pad_r = iw_end - in_width + 1
                                    else:
                                        slice_pad_r = 0
                                    ...
                                    deq_en = (tile_ic_end_in_tensor == in_channel - 1) and (
                                                slice_ci1_start_in_tile + slice_ci1_size == ci1_size_tile)
                                    ifm_M1K1M0K0 = C1BHWC02M1K1M0K0_L12L0(ifm_C1BHWC0_tile, batch, slice_oh_size,
                                                                          slice_ow_size, slice_ci1_size,
                                                                          slice_ih_start_in_tile,
                                                                          slice_iw_start_in_tile,
                                                                          slice_ci1_start_in_tile, kernel_h, kernel_w,
                                                                          stride_h, stride_w, slice_pad_t, slice_pad_b,
                                                                          slice_pad_l, slice_pad_r, dilation_h,
                                                                          dilation_w)
                                    weight_N1K1N0K0_slice = K1NK02N1K1N0K0_L12RMB(weight_k1nk0_tile, oc_size_tile,
                                                                                  slice_oc_start_in_tile,
                                                                                  slice_ci1_start_in_tile * kernel_h * kernel_w,
                                                                                  slice_oc_size,
                                                                                  slice_ci1_size * kernel_h * kernel_w)
                                    ...
                                    nb_oh = ceil_div(oh_size_tile, H_L0)
                                    nb_ow = ceil_div(ow_size_tile, W_L0)
                                    idx_oc = slice_oc_start_in_tile // CO_L0
                                    idx_oh = slice_oh_start_in_tile // H_L0
                                    idx_ow = slice_ow_start_in_tile // W_L0
                                    psb_addr = idx_oc * nb_oh * nb_ow + idx_oh * nb_ow + idx_ow

                                    result_m2n2m1n1m0n0_psb[psb_addr] = matmul_m1k1m0k0_n1k1n0k0( \
                                        result_m2n2m1n1m0n0_psb[psb_addr], ifm_M1K1M0K0, weight_N1K1N0K0_slice,
                                        bias_N1N0, deq_N1N0, ifm_M1K1M0K0.shape[0], weight_N1K1N0K0_slice.shape[0],
                                        ifm_M1K1M0K0.shape[1], bias_en, psum_en, deq_en)
                                    if (deq_en):
                                        ofm_BHWC = M1N1M0N02BHWC_PSB2DDR(ofm_BHWC, result_m2n2m1n1m0n0_psb[psb_addr],
                                                                         batch, out_height, out_width, out_channel,
                                                                         slice_oh_start_in_tensor,
                                                                         slice_ow_start_in_tensor,
                                                                         slice_oc_start_in_tensor, slice_oh_size,
                                                                         slice_ow_size, slice_oc_size)
    # 使用torch验证
    ...
    ofm_torch = nn.functional.conv2d(ifm_torch, weight_torch, bias_torch, stride=(stride_h, stride_w),
